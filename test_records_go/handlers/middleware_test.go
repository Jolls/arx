...
func TestRequireAuth_RedirectsWhenNoDB(t *testing.T) {
	...
	var reached bool
	req := httptest.NewRequest(http.MethodGet, "/form/1", nil)
...
func TestRequireCsrfOnPost_RejectsBadPost(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			var reached bool
			req := httptest.NewRequest(http.MethodPost, "/form/1", strings.NewReader(c.body))
...
func TestRequireCsrfOnPost_AllowsValidPost(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/form/1", strings.NewReader(body))
	req.Header.Set("Content-Type", "application/x-www-form-urlencoded")
