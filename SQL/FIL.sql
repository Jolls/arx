-- part_attachment: File and URL attachments linked to a part_number.
-- Each row is one attachment. part_id links to part_number.id.
...
-- TRIGGER: trg_FIL_part_count fires after INSERT/UPDATE/DELETE and updates part_number.attachment_count (active rows only).
...
CREATE TABLE part_attachment (
  ...
  part_id        INT NOT NULL  REFERENCES dbo.part_number (id),  -- FK to part_number.id.
