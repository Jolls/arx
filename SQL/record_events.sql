-- record_events: Audit trail for state changes on form_record (locked, unlocked, archived, activated, etc.).
-- test_record_id FKs to form_record.id.

IF OBJECT_ID('dbo.record_events', 'U') IS NOT NULL DROP TABLE record_events;

CREATE TABLE record_events (
  id             INT          PRIMARY KEY IDENTITY,
  test_record_id INT          NOT NULL REFERENCES dbo.form_record(id),  -- FK to form_record.id.
