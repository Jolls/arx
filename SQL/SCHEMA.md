...
| `users` | `id` | App-level user accounts. `username` = login handle (unique, case-insensitive in practice). `display_name` = shown in the header and written to audit events. `password_hash` = bcrypt. `is_active = 0` = soft-deactivated (cannot log in). The app sets `CONTEXT_INFO` to `username` before trigger-firing writes so `trg_test_definition_history` records the app user instead of `SYSTEM_USER`. See issue #461 for the planned cleanup once all binaries carry F1. |
