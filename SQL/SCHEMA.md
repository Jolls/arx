...
| 8301-8399 | `lot` | Lot control (#676): purchased lot 8301 of part 3007 (`lot_number` = its own id per #687, `lot_description` = "PO 5003", vendor lot captured, → `po_line` 5504), manufactured lot 8302 of sub-assembly 3012 (`lot_description` = "Build #8202"), manually-adjusted lot 8303 of 3007 (no `po_line_id`, no owning build — the third lot origin, #737 PRE-state row for the future `lot.source` enum), and manufactured lot 8306 of top-level assembly 3013 (`lot_description` = "Build #8203", #737). Parts 3007, 3012 + 3013 are `is_lot_tracked = 1`; 3005 (and its own BOM components 3002/3003) are left un-tracked so the #675 build test and existing app-driven build integration tests are unaffected |
| 8401-8499 | `lot_genealogy` | Lot control (#676): lot 8302 (3012) consumed 1 unit of lot 8301 (3007) (edge 8401); #737 adds edges 8402 (8302→8306) and 8403 (8303→8306) — top-level lot 8306 (3013) consumed both the sub-assembly lot and a second raw lot directly, so a recursive trace from 8306 resolves back through a two-level, two-parent tree (8301→8302→8306, and 8303→8306) |
| 8501-8599 | `unit` | Tier-3 serialized instances (#740, additive/inert until slice 8): 8501 = final-tested serial of top assembly 3013 with BOTH lot 8306 and build 8203 set; 8502 = serial of raw 3007 received inside purchased lot 8301 (lot only); 8503 = serial of assembly 3005 from non-lot-tracked build 8201 (build only). The three rows cover every branch of the `CK_unit_provenance` invariant (lot_id OR build_id) |
...
| `trg_form_row_history` | `form_row` | Snapshots old row values into `form_row_history` AFTER UPDATE (audit trail). Defined in `SQL/triggers.sql` alongside the count-maintenance triggers. |
...
| `contact` | `id` | Contacts, linked to companies. `company_id` → `company.id`. `is_active = 0` = inactive. (Renamed from `CN` in db-table-rename commit 1; Go struct fields were aligned to the columns in #693. `user_account_link` column dropped in #534/#540 — see `SQL/migrations/migrate_drop_contact_user_account_link.sql`.) |
...
| `unit` | `id` | Tier-3 serialized instance of a part — the serial-number tier (#740, traceability epic #736 slice 3). A serial number is a real row with FKs, not a parsed string. `part_id` → `part.id` (the part this unit is an instance of). `lot_id` → `lot.id` (nullable) = the lot a serialized unit lives in (`lot_serial` parts); `build_id` → `build.id` (nullable) = the build that produced it. Both nullable individually, but `CK_unit_provenance` (`lot_id IS NOT NULL OR build_id IS NOT NULL`) enforces every unit traces to at least one — the Q5 provenance invariant, backed by app-level assignment at creation. `serial_number` is a STRING (non-numeric serials allowed), `UNIQUE` per `part_id` (`UQ_unit_serial`), editable until the unit's first locked `form_record`. `is_active` = soft-delete/scrap flag. Created lazily one at a time when an individual unit is tested (Q5) — a build of 20 does not pre-create 20 rows; exists only for `serial`/`lot_serial` tracking. Additive/unused until slice 8 wires create/render logic. Run `SQL/migrations/migrate_740_unit_table.sql` to add the table to existing databases. |
...
| `form` | `id` | Test form definitions (renamed from `Forms` in db-table-rename commit 7; Go struct fields still use old names). `part_number_id` → `part.id`. `is_locked`/`is_active` flags. `test_order` = comma-separated `form_row.id` list. `instrument_types` = comma-separated valid instrument types for this form; drives the Instrument Type dropdown on records. `revision` = integer count of times this form has been released (locked); bumped on every unlock→lock transition, never on save; 0 = never released ("Draft") (#260). |
