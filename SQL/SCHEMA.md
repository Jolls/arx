...
columns, one domain at a time: `part`, `part_attachment`, and `bom` are done; `contact`,
`po_line`, and the test-record tables still carry old abbreviation-prefixed field names
(e.g. `POLQty`, `CNName`) until their PR lands. See the per-table notes in "Table reference"
