...
| `po_line` | `id` | PO line items → `purchase_order.id` via `po_id`; `part_id` → `part.id`. `part_number_snapshot`/`revision_snapshot` denormalize the part at order time. `lead_time_days` (issue #270) = supplier's quoted lead time, captured per line on the RFQ comparison grid; NULL = not quoted. Writes fire `trg_POL_part_count`. (Renamed from `POL` in db-table-rename commit 5; Go struct fields still use old `POL`-prefixed names.) |
