...
| `record_events` | `id` | Audit trail for state changes on `form_record` (locked, unlocked, archived, activated, etc.). `test_record_id` → `form_record.id`. `username`: OS username pre-#207, real app username from #207 (cutover 2026-06-19, v0.5.24) onward. Pre-cutover rows carrying legacy/garbage identifiers were audited and normalized where identifiable — see #630. |
