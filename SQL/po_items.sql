...
-- part_id links to part_number.id (the ordered part). part_number_snapshot is denormalized for historical reference.
-- vendor_part_number is the supplier's own part number for this item (see also supplier_part.supplier_pn).
-- TRIGGER: trg_POL_part_count fires after INSERT/UPDATE/DELETE and updates part_number.po_line_count.
...
CREATE TABLE po_line (
  ...
  revision_snapshot    VARCHAR(10),                  -- Denormalized revision at time of order. Snapshot of part_number.revision.
  part_id              INT,                           -- FK to part_number.id. Nullable — line items may not map to a catalog part.
...
ALTER TABLE dbo.po_line ADD CONSTRAINT FK_po_line_part FOREIGN KEY (part_id) REFERENCES dbo.part_number (id);
