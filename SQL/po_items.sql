...
-- part_id links to PN.PNID (the ordered part). part_number_snapshot is denormalized for historical reference.
...
CREATE TABLE po_line (
  ...
  revision_snapshot    VARCHAR(10),                  -- Denormalized revision at time of order. Snapshot of PN.revision.
  part_id              INT,                           -- FK to PN.PNID. Nullable — line items may not map to a catalog part.
...
ALTER TABLE dbo.po_line ADD CONSTRAINT FK_po_line_part FOREIGN KEY (part_id) REFERENCES dbo.PN (PNID);
