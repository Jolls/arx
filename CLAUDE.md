...
New table → update all 4 or test mode breaks: 1) `SQL/azure/<table>.sql` DDL (same schema prod+ArxDev) 2) `SQL/azure/seed_test_data.sql` DELETE+fixed-ID INSERT block 3) `arxlib/config/config.go` add `*Table()` helper 4) `SQL/schema.md` table reference section.

Every schema change ships a migration in `SQL/azure/migrations/migrate_<description>.sql` for a human to run — SQL/azure/*.sql is reference DDL, never auto-run. Migrations idempotent/guarded (e.g. `IF COL_LENGTH(...) IS NULL`). Pin to ArxDev: `USE ArxDev;` at top + comment that human changes it for ArxProd. Never default a migration to ArxProd. You author these, never run them (see ArxProd rule).
