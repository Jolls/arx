...
2. **Triggers - reopen the design rather than port 1:1. Confirmed.** The trigger sub-step reviews which of the 5 are still needed and whether survivors should become native Postgres or app-side logic instead of triggers (they were an Azure-era default). The per-trigger review happens at the start of Phase 2. **Review outcome (#670):** all 5 are still needed and stay triggers - a trigger is the idiomatic Postgres pattern for both the 4 denormalized counts and the audit snapshot. Ported in `SQL/postgres/triggers.sql` as statement-level triggers with transition tables; the audit trigger reads the actor from the `arx.username` session GUC (set by the dialect's `SetAuditUser`) instead of `CONTEXT_INFO()`. Generalizing the audit trigger for additional tables is deferred to #669 (v0.7).
